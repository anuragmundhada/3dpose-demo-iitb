...
function transformFloat(pt, center, scale, rot, res, invert)
...
function getTransform3D(center, scale, rot, res)
    -- center is a 2D point
    local h = 200 * scale
    local t = torch.eye(4)
    ...
    -- Translation
    t[1][4] = res * (-center[1] / h + .5)
    t[2][4] = res * (-center[2] / h + .5)
    ...
    if rot ~= 0 then
        ...
        r[2][2] = c
        -- Need to make sure rotation is around center
        local t_ = torch.eye(4)
        t_[1][4] = -res/2
        t_[2][4] = -res/2
        local t_inv = torch.eye(4)
        t_inv[1][4] = res/2
        t_inv[2][4] = res/2
        t = t_inv * r * t_ * t
    end
...
function transform3D(pt, center, scale, rot, res, invert)
    local pt_ = torch.ones(4)
    pt_[1], pt_[2], pt_[3] = pt[1] - 1, pt[2] - 1, pt[3] - 1

    local t = getTransform3D(center, scale, rot, res)
    if invert then
        t = torch.inverse(t)
    end
    local new_point = (t*pt_):sub(1,3)
...
function transform3DFloat(pt, center, scale, rot, res, invert)
    local pt_ = torch.ones(4)
    pt_[1], pt_[2], pt_[3] = pt[1] - 1, pt[2] - 1, pt[3] - 1
    local t = getTransform3D(center, scale, rot, res)
    if invert then
